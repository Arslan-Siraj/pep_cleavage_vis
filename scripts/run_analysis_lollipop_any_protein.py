...
def load_alias_map(alias_map: Optional[Path], query: str) -> Tuple[List[str], Optional[int], Optional[str], Optional[str]]:
    ...
    Supported columns are flexible. Recommended columns:
      target, aliases, length, display_name, output_prefix
...
def make_target_spec(
    ...
    for source in [map_aliases, aliases or []]:
        for value in source:
            for alias in split_alias_string(value):
                if normalize_match_text(alias) != normalize_match_text(query):
                    merged_aliases.append(alias)

    if normalize_match_text(query) == normalize_match_text(DEFAULT_TARGET):
        merged_aliases.extend(DEFAULT_ALIASES)
    ...
    display = map_display or " / ".join([query, *deduped[:5]])
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help=(
            "Target protein/gene/name to search. Examples: AT3G09260.1, Q9SR37, "
            "PYK10, BGLU23. Default keeps the original PYK10 analysis."
        ),
    )
    parser.add_argument(
        "--aliases",
        ...
        default=[],
        help=(
            "Additional names/identifiers for the same protein. Separate values with spaces, "
            "or use comma/semicolon-separated strings. Example: --aliases Q9SR37 PYK10 BGLU23"
        ),
    )
    parser.add_argument(
        "--alias-map",
