...
def read_metacaspase(path: Path) -> pd.DataFrame:
    """
    Read the ProteinBased_www_P10'P10 metacaspase matrix and retain PYK10 rows.

    This workbook has multi-row labels. The real column header is usually row 4.
    """
    wb = load_workbook(path, read_only=True, data_only=True)
    ...
    for r in ws.iter_rows(min_row=header_row + 1, values_only=True):
        ...
        is_target = (
            str(prot).upper() == TARGET_PROTEIN.upper()
            or re.search(r"PYK10|BGLU23|PSR3|beta-glucosidase", str(desc), flags=re.I) is not None
        )
    ...
    out = pd.DataFrame(rows)
    if out.empty:
        print("WARNING: no PYK10/BGLU23 rows found in metacaspase matrix.")
        return out
...
def build_lollipop_points(semi: pd.DataFrame, hunter: pd.DataFrame, meta: pd.DataFrame) -> pd.DataFrame:
    """
...
def main() -> None:
    ...
    points = build_lollipop_points(sem, hunter, meta)
